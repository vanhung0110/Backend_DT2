...
public class RoomWsHandler extends TextWebSocketHandler {
    ...
    private final com.example.hungdt2.rt.RtRoomService rtRoomService;

    public RoomWsHandler(com.example.hungdt2.rt.RtRoomService rtRoomService) {
        this.rtRoomService = rtRoomService;
    }
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        log.info("WS open: sessionId={}, roomId={}, userId={}, attrs={}", session.getId(), roomId, userIdStr, session.getAttributes());
    }
    ...
    public void sendToRoom(String roomId, java.util.Map<String, Object> payload) {
        try {
            ...
            for (WebSocketSession s : list) {
                try { if (s.isOpen()) s.sendMessage(msg); } catch (Exception e) { log.warn("Failed to send WS msg to session {}: {}", s.getId(), e.getMessage()); }
            }
        } catch (Exception ex) {
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        // Remove from per-user tracking and only mark offline when no more sessions for that user in the room
        try {
            Object uidObj = session.getAttributes().get("userId");
            String userIdStr = String.valueOf(uidObj);
            Long userId = null;
            try { userId = Long.valueOf(userIdStr); } catch (Exception ex) { userId = null; }
            String roomId = (String) session.getAttributes().get("roomId");
            if (roomId == null) {
                String path = session.getUri().getPath(); // /ws/rooms/{id}
                String[] p = path.split("/");
                ...
                if (usersMap != null) {
                    ...
                    if (userSet != null) {
                        ...
                        if (userSet.isEmpty()) {
                            ...
                            try { rtRoomService.onWsDisconnect(Long.valueOf(roomId), userId); } catch (Exception ex) { log.warn("Failed to process ws disconnect: {}", ex.getMessage()); }
                        }
                    }
                    if (usersMap.isEmpty()) {
